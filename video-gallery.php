...
/*
  ...
  Plugin URI: http://www.gurkanoluc.com/video-gallery-plugin
...
class videoGallery {
	...
	public function mainContent() {
		...
		$func = $_GET['f'];
		$p = $_GET['p'];
		// Dirty hack for js i18n
		__('Kapat', 'videogallery');
...
EOS;
    ...
    video_width = %d, video_height = %d, thumbnail_width = %d, thumbnail_height = %d", $gallery_name,
						$video_width, $video_height, $thumbnail_width, $thumbnail_height);
		...
		echo $this->_ajaxResponse(array(1, $this->_('Galeri oluşturuldu'), 'gallery'));
		return;
	}
    ...
    video_height = %d, thumbnail_width = %d, thumbnail_height = %d WHERE id= %d",
						$gallery_name, $video_width, $video_height, $thumb_width, $thumb_height, $gallery_id);
		$query = $this->db->query($sql);
    ...
    class="gallery_edit_add_video_button button-secondary" value="' . $this->_('Video Ekle') . '"/>';
		...
		if ($videos) {
			...
			foreach ($videos AS $video) {
				...
				$html .= '<br/><span class="gallery_edit_video_delete_link">
